using System;
using System.Collections.Generic;

#nullable disable

namespace ISPTF.Models
{
    public partial class Sheet1_
    {
        public string FcdAccNo { get; set; }
        public string FcdEntryDate { get; set; }
        public string TranSeqNo { get; set; }
        public string TranDoc { get; set; }
        public string RefTranDoc { get; set; }
        public string TranCode { get; set; }
        public string TranDept { get; set; }
        public string RecStatus { get; set; }
        public string TranFcdStatus { get; set; }
        public string FlagReverse { get; set; }
        public string RefAccount { get; set; }
        public string FcdCross { get; set; }
        public string EventDate { get; set; }
        public string FcdBalance { get; set; }
        public string PrevFcdBal { get; set; }
        public string FcdAmt { get; set; }
        public string HoldAmt { get; set; }
        public string CustCode { get; set; }
        public string FcdCcy { get; set; }
        public string ExchRate { get; set; }
        public string FcdInterest { get; set; }
        public string FromDate { get; set; }
        public string ToDate { get; set; }
        public string IntRate { get; set; }
        public string IntSpread { get; set; }
        public string TranFDepos { get; set; }
        public string TranFTel { get; set; }
        public string Mixpayment { get; set; }
        public string Paytype { get; set; }
        public string TranFMethod { get; set; }
        public string TranFcdAmt { get; set; }
        public string TranFCashAmt { get; set; }
        public string TranChquAmt { get; set; }
        public string TranDrAmt { get; set; }
        public string Remark { get; set; }
        public string Allocation { get; set; }
        public string ForwardNo { get; set; }
        public string RelateCode { get; set; }
        public string GoodsCode { get; set; }
        public string PurposeCode { get; set; }
        public string ChqNo { get; set; }
        public string ChqBank { get; set; }
        public string ChqBran { get; set; }
        public string ExchRate2 { get; set; }
        public string THBBal { get; set; }
        public string CurrentAmt { get; set; }
        public string AccNo1 { get; set; }
        public string AccNo2 { get; set; }
        public string AccNo3 { get; set; }
        public string AccAmt1 { get; set; }
        public string AccAmt2 { get; set; }
        public string AccAmt3 { get; set; }
        public string ReceiptNo { get; set; }
        public string LastReceiptNo { get; set; }
        public string DateLastPaid { get; set; }
        public string MaintenAmt { get; set; }
        public string IntAmt { get; set; }
        public string ProfitAmt { get; set; }
        public string CommFCD { get; set; }
        public string CommOther { get; set; }
        public string CommBnet { get; set; }
        public string CommCertify { get; set; }
        public string TaxRefund { get; set; }
        public string TaxAmt { get; set; }
        public string PayFlag { get; set; }
        public string FcdAccTerm { get; set; }
        public string DueDate { get; set; }
        public string VoucherID { get; set; }
        public string UpdateDate { get; set; }
        public string UserCode { get; set; }
        public string AuthDate { get; set; }
        public string AuthCode { get; set; }
        public string CenterID { get; set; }
        public string F74 { get; set; }
        public string F75 { get; set; }
        public string F76 { get; set; }
        public string F77 { get; set; }
        public string F78 { get; set; }
        public string F79 { get; set; }
        public string F80 { get; set; }
        public string F81 { get; set; }
        public string F82 { get; set; }
        public string F83 { get; set; }
        public string F84 { get; set; }
        public string F85 { get; set; }
        public string F86 { get; set; }
        public string F87 { get; set; }
        public string F88 { get; set; }
        public string F89 { get; set; }
        public string F90 { get; set; }
        public string F91 { get; set; }
        public string F92 { get; set; }
        public string F93 { get; set; }
        public string F94 { get; set; }
        public string F95 { get; set; }
        public string F96 { get; set; }
        public string F97 { get; set; }
        public string F98 { get; set; }
        public string F99 { get; set; }
        public string F100 { get; set; }
        public string F101 { get; set; }
        public string F102 { get; set; }
        public string F103 { get; set; }
    }
}
