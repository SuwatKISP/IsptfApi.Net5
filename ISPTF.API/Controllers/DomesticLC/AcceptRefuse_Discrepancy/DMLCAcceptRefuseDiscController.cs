...
namespace ISPTF.API.Controllers.DomesticLC
{
    ...
    public class DMLCAcceptRefuseDiscController : ControllerBase
    {
        ...
        public DMLCAcceptRefuseDiscController(ISqlDataAccess db, ISPTFContext context)
        {
            _db = db;
            _context = context;
        }
        ...
        [HttpGet("list")]
        public async Task<ActionResult<AcceptRefuseDiscListPageResponse>> List(string? ListType,string? CenterID, string? @BENumber, string? CustCode, string? @CustName, string? Page, string? PageSize)
        {
            AcceptRefuseDiscListPageResponse response = new AcceptRefuseDiscListPageResponse();
            ...
            if (string.IsNullOrEmpty(ListType) || string.IsNullOrEmpty(CenterID) || string.IsNullOrEmpty(Page) || string.IsNullOrEmpty(PageSize))
            {
                ...
                response.Data = new List<Q_AcceptRefuseDiscListPageRsp>();
            ...
            try
            {
                ...
                var results = await _db.LoadData<Q_AcceptRefuseDiscListPageRsp, dynamic>(
                            storedProcedure: "usp_q_DMLC_AccdptRefuseDiscListPage",
                            param);

                response.Code = Constants.RESPONSE_OK;
                response.Message = "Success";
                response.Data = (List<Q_AcceptRefuseDiscListPageRsp>)results;
        ...
        [HttpGet("select")]
        public async Task<ActionResult<Q_DMLC_AcceptRefuseDisc_Select_Response>> ListSelect(string? ListType, string? BENumber, int? BESeqno, string? RecType, string? Event, string? CustCode)
        {
            Q_DMLC_AcceptRefuseDisc_Select_Response response = new Q_DMLC_AcceptRefuseDisc_Select_Response();
            var USER_ID = User.Identity.Name;
            //var USER_ID = "API";
            // Validate
            if (string.IsNullOrEmpty(ListType))
            {
                ...
                response.Data = new Q_DMLC_AcceptRefuseDisc_Select_JSON_rsp();
                return BadRequest(response);
            }
            ...
            try
            {
                ...
                var results = await _db.LoadData<Q_DMLC_AcceptRefuseDisc_Select_JSON_rsp, dynamic>(
                            storedProcedure: "[usp_q_DMLC_AccdptRefuseDiscListSelect]",
                            param);
                ...
                if (Resp == 1)
                {
                    Q_DMLC_AcceptRefuseDisc_Select_JSON_rsp jsonResponse = JsonSerializer.Deserialize<Q_DMLC_AcceptRefuseDisc_Select_JSON_rsp>(SelectResp);
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "Success";
        ...
        [HttpPost("save")]
        public async Task<ActionResult<DMLC_SaveAcceptRefuseDisc_Response>> Save([FromBody] DMLC_SaveAcceptRefuseDisc_JSON_req save)
        {
            DMLC_SaveAcceptRefuseDisc_Response response = new();
            var USER_ID = User.Identity.Name;
            ...
            if (string.IsNullOrEmpty(save.ListType.ListType))
            {
                response.Code = Constants.RESPONSE_FIELD_REQUIRED;
                response.Message = "ListType is required";
                response.Data = new DMLC_SaveAcceptRefuseDisc_JSON_rsp();
                return BadRequest(response);
            }
            ...
            try
            {
                ...
                var results = await _db.LoadData<DMLC_SaveAcceptRefuseDisc_JSON_rsp, dynamic>(
                    storedProcedure: "usp_pDOMBE_AcceptRefuse_Save",
                    param);
                ...
                if (Resp > 0)
                {
                    DMLC_SaveAcceptRefuseDisc_JSON_rsp jsonResponse = JsonSerializer.Deserialize<DMLC_SaveAcceptRefuseDisc_JSON_rsp>(SaveResp);
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "Success";
        ...
        [HttpPost("release")]
        public async Task<ActionResult<DMLCResultResponse>> Release([FromBody] DMLC_ReleaseAcceptRefuseDisc_JSON_req release)
        {
            ...
            param.Add("@AdviceResult", release.pDOMBE.AdviceResult);
            param.Add("@UserCode", USER_ID);
            param.Add("@CenterID", USER_CENTER_ID);
            ...
            {
                await _db.SaveData(
                  storedProcedure: "usp_pDOMBE_AcceptRefuse_Release", param);
                var resp = param.Get<int>("@Resp");
        ...
        [HttpPost("delete")]
        public async Task<ActionResult<DMLCResultResponse>> delete([FromBody] DMLC_DeleteAcceptRefuseDisc_JSON_req delete)
        {
            DMLCResultResponse response = new();
            var USER_ID = User.Identity.Name;
            var claimsPrincipal = HttpContext.User;
            ...
            {
                await _db.SaveData(
                  storedProcedure: "usp_pDOMBE_AcceptRefuse_Delete", param);
                var resp = param.Get<int>("@Resp");
