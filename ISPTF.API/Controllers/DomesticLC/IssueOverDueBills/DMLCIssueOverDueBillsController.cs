...
namespace ISPTF.API.Controllers.DomesticLC
{
    ...
    public class DMLCIssueOverDueBillsController : ControllerBase
    {
        ...
        [HttpPost("save")]
        public async Task<ActionResult<DMLC_SaveIssueOverDueBills_Response>> Save([FromBody] DMLC_SaveIssueOverDueBills_JSON_req save)
        {
            DMLC_SaveIssueOverDueBills_Response response = new();
            var USER_ID = User.Identity.Name;
            ...
            if (string.IsNullOrEmpty(save.ListType.ListType))
            {
                response.Code = Constants.RESPONSE_FIELD_REQUIRED;
                response.Message = "ListType is required";
                response.Data = new DMLC_SaveIssueOverDueBills_JSON_rsp();
                return BadRequest(response);
            }
            ...
            try
            {
                ...
                param.Add("@TxIntDay", save.ListType.TxIntDay);
                param.Add("@TxCCyInt", save.ListType.TxCCyInt);
                param.Add("@TxBhtInt", save.ListType.TxBhtInt);
                ...
                var results = await _db.LoadData<DMLC_SaveIssueOverDueBills_JSON_rsp, dynamic>(
                    storedProcedure: "usp_pDOMBE_IssueOverdueBills_Save",
                    param);
                ...
                if (Resp > 0)
                {
                    DMLC_SaveIssueOverDueBills_JSON_rsp jsonResponse = JsonSerializer.Deserialize<DMLC_SaveIssueOverDueBills_JSON_rsp>(SaveResp);
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "Success";
        ...
        [HttpPost("release")]
        public async Task<ActionResult<DMLCResultResponse>> Release([FromBody] DMLC_ReleaseIssueOverDueBills_JSON_req release)
        {
            ...
            param.Add("@TxDraftAmt", release.ListType.TxDraftAmt);
            param.Add("@TxExchRate", release.ListType.TxExchRate);
            param.Add("@TxCCyInt", release.ListType.TxCCyInt);
            param.Add("@TxAccInt", release.ListType.TxAccInt);
            param.Add("@TxBhtInt", release.ListType.TxBhtInt);

            //pDOMBE
            //param.Add("@", release.pDOMBE.);
            param.Add("@BENumber", release.pDOMBE.BENumber);
            param.Add("@BESeqno", release.pDOMBE.BESeqno);
            param.Add("@OverDueDate", release.pDOMBE.OverDueDate);
            ...
            {
                await _db.SaveData(
                  storedProcedure: "usp_pDOMBE_IssueOverdueBills_Release", param);
                var resp = param.Get<int>("@Resp");
        ...
        [HttpPost("delete")]
        public async Task<ActionResult<DMLCResultResponse>> delete([FromBody] DMLC_DeleteIssueOverDueBills_JSON_req delete)
        {
            DMLCResultResponse response = new();
            var USER_ID = User.Identity.Name;
            var claimsPrincipal = HttpContext.User;
            ...
            {
                await _db.SaveData(
                  storedProcedure: "usp_pDOMBE_IssueOverdueBills_Delete", param);
                var resp = param.Get<int>("@Resp");
