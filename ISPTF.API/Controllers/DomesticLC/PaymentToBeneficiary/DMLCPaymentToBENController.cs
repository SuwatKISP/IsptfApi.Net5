...
namespace ISPTF.API.Controllers.DomesticLC
{
    ...
    public class DMLCPaymentToBENController : ControllerBase
    {
        ...
        [HttpGet("select")]
        public async Task<ActionResult<Q_DMLC_PaymentToBEN_Select_Response>> ListSelect(string? ListType, string? BENumber, int? BESeqno, string? RecType, string? Event, string? CustCode)
        {
            Q_DMLC_PaymentToBEN_Select_Response response = new Q_DMLC_PaymentToBEN_Select_Response();
            var USER_ID = User.Identity.Name;
            //var USER_ID = "API";
            // Validate
            if (string.IsNullOrEmpty(ListType))
            {
                ...
                response.Data = new Q_DMLC_PaymentToBEN_Select_JSON_rsp();
                return BadRequest(response);
            }
            ...
            try
            {
                ...
                var results = await _db.LoadData<Q_DMLC_PaymentToBEN_Select_JSON_rsp, dynamic>(
                            storedProcedure: "usp_q_DMLC_PaymentToBENSelect",
                            param);
                ...
                if (Resp == 1)
                {
                    Q_DMLC_PaymentToBEN_Select_JSON_rsp jsonResponse = JsonSerializer.Deserialize<Q_DMLC_PaymentToBEN_Select_JSON_rsp>(SelectResp);
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "Success";
        ...
        [HttpPost("save")]
        public async Task<ActionResult<DMLC_SavePaymentToBEN_Response>> Save([FromBody] DMLC_SavePaymentToBEN_JSON_req save)
        {
            DMLC_SavePaymentToBEN_Response response = new();
            var USER_ID = User.Identity.Name;
            ...
            if (string.IsNullOrEmpty(save.ListType.ListType))
            {
                response.Code = Constants.RESPONSE_FIELD_REQUIRED;
                response.Message = "ListType is required";
                response.Data = new DMLC_SavePaymentToBEN_JSON_rsp();
                return BadRequest(response);
            }
            ...
            try
            {
                ...
                var results = await _db.LoadData<DMLC_SavePaymentToBEN_JSON_rsp, dynamic>(
                    storedProcedure: "usp_pDOMBE_PaymentToBEN_Save",
                    param);
                ...
                if (Resp > 0)
                {
                    DMLC_SavePaymentToBEN_JSON_rsp jsonResponse = JsonSerializer.Deserialize<DMLC_SavePaymentToBEN_JSON_rsp>(SaveResp);
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "Success";
        ...
        [HttpPost("release")]
        public async Task<ActionResult<DMLCResultResponse>> Release([FromBody] DMLC_ReleasePaymentToBEN_JSON_req release)
        {
            ...
            param.Add("@TxDraftBht", release.ListType.TxDraftBht);
            ...
            param.Add("@PaymentFlag", release.pDOMBE.PaymentFlag);
            ...
            {
                await _db.SaveData(
                  storedProcedure: "usp_pDOMBE_PaymentToBEN_Release", param);
                var resp = param.Get<int>("@Resp");
        ...
        [HttpPost("delete")]
        public async Task<ActionResult<DMLCResultResponse>> delete([FromBody] DMLC_DeletePaymentToBEN_JSON_req delete)
        {
            DMLCResultResponse response = new();
            var USER_ID = User.Identity.Name;
            var claimsPrincipal = HttpContext.User;
            ...
            {
                await _db.SaveData(
                  storedProcedure: "usp_pDOMBE_PaymentToBEN_Delete", param);
                var resp = param.Get<int>("@Resp");
