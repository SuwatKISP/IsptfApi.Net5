...
namespace ISPTF.API.Controllers.LoginRegis
{
    ...
    public class AMSBLCpDocregisterController : ControllerBase
    {
        private readonly ISqlDataAccess _db;
        public AMSBLCpDocregisterController(ISqlDataAccess db)
        {
            _db = db;
        }
        //[HttpGet("select")]
        ...
        public async Task<IEnumerable<Q_GetDocRegisterPagebyModuleRsp>> GetAll(string? LogType, string? CenterID, string? RegDocNo, string? CustCode, string? CustName, string? Page, string? PageSize)
        {
            ...
            var results = await _db.LoadData<Q_GetDocRegisterPagebyModuleRsp, dynamic>(
                        storedProcedure: "usp_q_GetDocRegisterSelectPageAMSBLC",
                        param);
            return results;
        }

        [HttpGet("register/referLCnum")]
        public async Task<IEnumerable<Q_GetDocRegisterReferLCNoAMSBLCRsp>> GetAllRefLC(string? CenterID, string? SBLCNumber, string? CustCode, string? CustName, string? Page, string? PageSize)
        {
            DynamicParameters param = new();
            ...
            var results = await _db.LoadData<Q_GetDocRegisterReferLCNoAMSBLCRsp, dynamic>(
                        storedProcedure: "usp_q_GetDocRegisterReferLCPageAMSBLC",
                        param);
            return results;
        }
        ...
        public async Task<ActionResult<List<PDocRegisterInsUpdReq>>> Insert([FromBody] PDocRegisterInsUpdReq pdocregisterreq)
        {
            ...
            {
                var results = await _db.LoadData<PDocRegister, dynamic>(
                    storedProcedure: "usp_pDocRegisterAMSBLCInsert",
                    param);
        ...
        public async Task<ActionResult<List<PDocRegisterInsUpdReq>>> Update([FromBody] PDocRegisterInsUpdReq pdocregisterreq)
        {
            ...
            {
                var results = await _db.LoadData<PDocRegister, dynamic>(
                    storedProcedure: "usp_pDocRegisterAMSBLCUpdate",
                    param);
                var resp = param.Get<int>("@Resp");
                if (resp == 1)
                {
        ...
        public async Task<ActionResult<List<PDocRegisterInsUpdReq>>> Cancel([FromBody] PDocRegisterInsUpdReq pdocregisterreq)
        {
            ...
            {
                var results = await _db.LoadData<PDocRegister, dynamic>(
                    storedProcedure: "usp_pDocRegisterAMSBLCCancel",
                    param);
                var resp = param.Get<int>("@Resp");
                if (resp == 1)
                {
        ...
        [HttpGet("grdadvice")]
        public async Task<IEnumerable<PSBLCGrdAdviceRsp>> GetGrdAdvice(string? CustCode, string? SBLCStatus, string? RecStatus, string? RecType,   string? CenterID)
        {
            DynamicParameters param = new();

            param.Add("@CustCode", CustCode);
            param.Add("@CenterID", CenterID);
            param.Add("@RecType", RecType); // = M
            param.Add("@SBLCStatus", SBLCStatus); // = A
            param.Add("@RecStatus", RecStatus); // = R
            ...
            var results = await _db.LoadData<PSBLCGrdAdviceRsp, dynamic>(
                        storedProcedure: "usp_pSBLC_GrdAdvice_Select",
                        param);
            return results;
        }
        ...
        //                storedProcedure: "usp_q_GetDocRegisterIssuedAMSBLC",
        //                param);
        //    return results;
        //}
