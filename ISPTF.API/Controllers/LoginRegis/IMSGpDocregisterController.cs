...
namespace ISPTF.API.Controllers.LoginRegis
{
    ...
    public class IMSGpDocregisterController : ControllerBase
    {
        private readonly ISqlDataAccess _db;
        public IMSGpDocregisterController(ISqlDataAccess db)
        {
            _db = db;
        }
        //[HttpGet("select")]
        ...
        public async Task<IEnumerable<Q_GetDocRegisterPagebyModuleRsp>> GetAll(string? LogType, string? CenterID, string? RegDocNo, string? CustCode, string? CustName, string? Page, string? PageSize)
        {
            ...
            var results = await _db.LoadData<Q_GetDocRegisterPagebyModuleRsp, dynamic>(
                        storedProcedure: "usp_q_GetDocRegisterSelectPageIMSG",
                        param);
            return results;
        }
        ...
        public async Task<ActionResult<List<PDocRegisterInsUpdReq>>> Insert([FromBody] PDocRegisterInsUpdReq pdocregisterreq)
        {
            ...
            {
                var results = await _db.LoadData<PDocRegister, dynamic>(
                    storedProcedure: "usp_pDocRegisterIMSGInsert",
                    param);
        ...
        public async Task<ActionResult<List<PDocRegisterInsUpdReq>>> Update([FromBody] PDocRegisterInsUpdReq pdocregisterreq)
        {
            ...
            {
                var results = await _db.LoadData<PDocRegister, dynamic>(
                    storedProcedure: "usp_pDocRegisterIMSGUpdate",
                    param);
                var resp = param.Get<int>("@Resp");
                if (resp == 1)
                {
        ...
        public async Task<ActionResult<List<PDocRegisterInsUpdReq>>> Cancel([FromBody] PDocRegisterInsUpdReq pdocregisterreq)
        {
            ...
            {
                var results = await _db.LoadData<PDocRegister, dynamic>(
                    storedProcedure: "usp_pDocRegisterIMSGCancel",
                    param);
                var resp = param.Get<int>("@Resp");
                if (resp == 1)
                {
        ...
        [HttpGet("grdadvice")]
        public async Task<IEnumerable<PIMBLGrdAdviceRsp>> GetGrdAdvice(string? CustCode, string? RefLCNumber, string? RecType, string? BLStatus, string? RecStatus)
        {
            DynamicParameters param = new();

            param.Add("@CustCode", CustCode);    
            param.Add("@RefLCNumber", RefLCNumber);
            param.Add("@RecType", RecType); // = M
            param.Add("@BLStatus", BLStatus); // = A
            ...
            if (RefLCNumber == null)
            {
                param.Add("@RefLCNumber", "");
            }

            var results = await _db.LoadData<PIMBLGrdAdviceRsp, dynamic>(
                        storedProcedure: "usp_pIMBL_GrdAdvice_Select",
                        param);
            return results;
        }
        ...
        //                storedProcedure: "usp_q_GetDocRegisterIssuedIMSG",
        //                param);
        //    return results;
        //}
