...
namespace ISPTF.API.Controllers.ExportLC
{
    ...
    public class EXLCLCReverseOverDueController : ControllerBase
    {
        private readonly ISqlDataAccess _db;
        private readonly ISPTFContext _context;

        private const string BUSINESS_TYPE = "21";
        private const string EVENT_TYPE = "REACTIVE";

        public EXLCLCReverseOverDueController(ISqlDataAccess db, ISPTFContext context)
        {
            _db = db;
            _context = context;
        }
        ...
        public async Task<ActionResult<EXLCLCReverseOverDueListResponse>> GetAllList(string? @ListType, string? CenterID, string? EXPORT_LC_NO, string? BENName, string? USER_ID, string? Page, string? PageSize)
        {
            ...
            if (string.IsNullOrEmpty(ListType) || string.IsNullOrEmpty(CenterID) || string.IsNullOrEmpty(Page) || string.IsNullOrEmpty(PageSize))
            {
                ...
                response.Data = new List<Q_EXLCLCReverseOverDueListPageRsp>();
            ...
            try
            {
                ...
                var results = await _db.LoadData<Q_EXLCLCReverseOverDueListPageRsp, dynamic>(
                            storedProcedure: "usp_q_EXLC_LCReverseOverDueListPage",
                            param);
                response.Code = Constants.RESPONSE_OK;
                response.Message = "Success";
                response.Data = (List<Q_EXLCLCReverseOverDueListPageRsp>)results;
        ...
        [HttpGet("select")]
        public async Task<ActionResult<PEXLCRecordResponse>> GetAllSelect(string? EXPORT_LC_NO, string? EVENT_NO, string? LFROM)
        {
            ...
            try
            {
                ...
                var results = await _db.LoadData<PEXLCRecordRsp, dynamic>(
                           storedProcedure: "usp_pEXLC_LCReverseOverdue_Select",
                           param);
