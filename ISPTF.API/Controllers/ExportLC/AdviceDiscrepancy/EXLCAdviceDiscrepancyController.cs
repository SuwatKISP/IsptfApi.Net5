...
namespace ISPTF.API.Controllers.ExportLC
{
    ...
    public class EXLCAdviceDiscrepancyController : ControllerBase
    {
        ...
        [HttpGet("list")]
        public async Task<IEnumerable<Q_EXLCAdviceDiscrepancyListPageRsp>> GetAllList(string? @ListType, string? CenterID, string? EXPORT_LC_NO, string? BENName, string? USER_ID, string? Page, string? PageSize)
        {
        ...
        [HttpGet("select")]
        public async Task<ActionResult<PEXLCRecordRsp>> GetAllSelect(string? EXPORT_LC_NO, int? EVENT_NO, string? RECORD_TYPE, string? REC_STATUS, string? LFROM)
        {
            DynamicParameters param = new();
