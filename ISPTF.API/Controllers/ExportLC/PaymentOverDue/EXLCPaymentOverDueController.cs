...
namespace ISPTF.API.Controllers.ExportLC
{
    ...
    public class EXLCPaymentOverDueController : ControllerBase
    {
        ...
        public EXLCPaymentOverDueController(ISqlDataAccess db)
        {
            _db = db;
        }

        [HttpGet("list")]
        public async Task<IEnumerable<Q_EXLCPaymentOverDueListPageRsp>> GetAllList(string? @ListType, string? CenterID, string? EXPORT_LC_NO, string? BENName, string? USER_ID, string? Page, string? PageSize)
        {
        ...
        [HttpGet("select")]
        public async Task<ActionResult<PEXLCPEXPaymentRsp>> GetAllSelect(string? EXPORT_LC_NO, int? EVENT_NO,  string? LFROM)
        {
            DynamicParameters param = new();
