...
namespace ISPTF.API.Controllers.ExportBC
{
    ...
    public class EXBCReverseCollectionController : ControllerBase
    {
        private readonly ISqlDataAccess _db;
        public EXBCReverseCollectionController(ISqlDataAccess db)
        {
            _db = db;
        }
        ...
        public async Task<ActionResult<EXBCResultResponse>> Release([FromBody] PEXBCReverseCollectionReleaseReq PEXBCReversePurchRelease)
        {
            ...
            {
                await _db.SaveData(
                  storedProcedure: "usp_pEXBC_ReverseCollection_Release", param);
                //var resp = param.Get<int>("@Resp");
                var resp = param.Get<string>("@Resp");
                if (resp == "1")
                {
