...
namespace ISPTF.API.Controllers.ExportBC
{
    ...
    public class EXBCIssuePurchaseController : ControllerBase
    {
        ...
        [HttpGet("newlist")]
        public async Task<IEnumerable<Q_EXBCNewPageRsp>> GetAllNew(string? CenterID, string? RegDocNo, string? BENName, string? Page, string? PageSize)
        {
        ...
        [HttpGet("editlist")]
        public async Task<IEnumerable<Q_EXBCEditPageRsp>> GetAllEdit(string? FormType, string? USER_ID, string? CenterID, string? EXPORT_BC_NO, string? BENNAME, string? Page, string? PageSize)
        {
            DynamicParameters param = new();
