...
namespace ISPTF.API.Controllers.SBLC
{
    ...
    public class SBLCRegisterClaimController : ControllerBase
    {
        private readonly ISqlDataAccess _db;
        public SBLCRegisterClaimController(ISqlDataAccess db)
        {
            _db = db;
        }
        ...
        public async Task<IEnumerable<SBLCRegisterClaimListRsp>> GetList(string? ListType, string? CenterID,  string? SBLCNumber, string? CustCode, string? CustName, string? UserCode, string? Page, string? PageSize)
        {
            ...
            var results = await _db.LoadData<SBLCRegisterClaimListRsp, dynamic>(
                        storedProcedure: "usp_q_SBLC_RegisterClaimListPage",
                        param);
            return results;
        }
        ...
        public async Task<ActionResult<PSBLCPaymentRsp>> GetAllSelect(string SBLCNumber, string SBLCSeqno, string RecType, string EVENT, string RecStatus)
        {
            ...
            try
            {
                var results = await _db.LoadData<PSBLCPaymentRsp, dynamic>(
                           storedProcedure: "usp_pSBLC_RegisterClaim_Select",
                           param);
