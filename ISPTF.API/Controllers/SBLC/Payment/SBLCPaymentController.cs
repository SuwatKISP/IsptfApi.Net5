...
namespace ISPTF.API.Controllers.SBLC
{
    ...
    public class SBLCPaymentController : ControllerBase
    {
        private readonly ISqlDataAccess _db;
        public SBLCPaymentController(ISqlDataAccess db)
        {
            _db = db;
        }
        ...
        [HttpGet("list")]
        public async Task<IEnumerable<SBLCPaymentListRsp>> GetList(string? ListType, string? CenterID,  string? SBLCNumber, string? CustCode, string? CustName, string? UserCode, string? Page, string? PageSize)
        {
            ...
            var results = await _db.LoadData<SBLCPaymentListRsp, dynamic>(
                        storedProcedure: "usp_q_SBLC_PaymentListPage",
                        param);
            return results;
        }
        [HttpGet("select")]
        public async Task<ActionResult<PSBLCPaymentRsp>> GetAllSelect(string SBLCNumber, string SBLCSeqno, string RecType, string EVENT, string RecStatus)
        {
            ...
            try
            {
                var results = await _db.LoadData<PSBLCPaymentRsp, dynamic>(
                           storedProcedure: "usp_pSBLC_Payment_Select",
                           param);
