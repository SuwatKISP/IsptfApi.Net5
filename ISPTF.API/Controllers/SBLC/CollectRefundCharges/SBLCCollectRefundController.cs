...
namespace ISPTF.API.Controllers.SBLC
{
    ...
    public class SBLCCollectRefundController : ControllerBase
    {
        private readonly ISqlDataAccess _db;
        public SBLCCollectRefundController(ISqlDataAccess db)
        {
            _db = db;
        }
        ...
        [HttpGet("list")]
        public async Task<IEnumerable<SBLCCollectRefundListRsp>> GetList(string? ListType, string? CenterID,  string? SBLCNumber, string? CustCode, string? CustName, string? UserCode, string? Page, string? PageSize)
        {
            ...
            var results = await _db.LoadData<SBLCCollectRefundListRsp, dynamic>(
                        storedProcedure: "usp_q_SBLC_CollectRefundListPage",
                        param);
            return results;
        }
        [HttpGet("select")]
        public async Task<ActionResult<PSBLCPaymentRsp>> GetAllSelect(string SBLCNumber, string SBLCSeqno, string RecType, string EVENT, string RecStatus)
        {
            ...
            try
            {
                var results = await _db.LoadData<PSBLCPaymentRsp, dynamic>(
                           storedProcedure: "usp_pSBLC_CollectRefund_Select",
                           param);
