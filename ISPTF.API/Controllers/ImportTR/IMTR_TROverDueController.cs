...
namespace ISPTF.API.Controllers.ImportTR
{
    ...
    public class IMTR_TROverDueController : ControllerBase
    {
        private readonly ISqlDataAccess _db;
        private readonly ISPTFContext _context;
        public IMTR_TROverDueController(ISqlDataAccess db, ISPTFContext context)
        {
            _db = db;
            _context = context;
        }
        ...
        public async Task<ActionResult<Q_IMTR_ListPage_Response>> ListPage(string? ListType, string CustCode,string? CustName,string? TRNumber ,string? CenterID, string? Page, string? PageSize)
        {
            ...
            try
            {
                ...
                var results = await _db.LoadData<Q_IMTR_ListPage_rsp, dynamic>(
                            storedProcedure: "usp_q_IMTR_TROverDueListPage",
                            param);
        ...
        public async Task<ActionResult<Q_IMTR_TROverDueSelect_Response>> Select(string? ListType, string? CustCode, string? RefNumber, string? TRSeqno, string? RecType, string? Event)
        {
            ...
            try
            {
                ...
                param.Add("@TROverDueResp", dbType: DbType.String,
                           direction: System.Data.ParameterDirection.Output,
                           size: 5215585);


                var results = await _db.LoadData<Q_IMTR_TROverDueSelect_JSON_rsp, dynamic>(
                            storedProcedure: "usp_Q_IMTR_TROverDueListSelect",
                            param);

                var Resp = param.Get<dynamic>("@Resp");
                var TROverDueResp = param.Get<dynamic>("@TROverDueResp");

                if (Resp == 1)
                {
                    Q_IMTR_TROverDueSelect_JSON_rsp jsonResponse = JsonSerializer.Deserialize<Q_IMTR_TROverDueSelect_JSON_rsp>(TROverDueResp);
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "Success";
        ...
        [HttpPost("save")]
        public async Task<ActionResult<IMTR_SaveTROverDue_Response>> Save([FromBody] IMTR_SaveTROverDue_JSON_req save)
        {
            ...
            if (save.ListType.ListType != "NEW" && save.ListType.ListType != "EDIT")
            {
                ...
                response.Data = new IMTR_SaveTROverDue_JSON_rsp();
                return BadRequest(response);
            }
            try
            {
                ...
                var results = await _db.LoadData<IMTR_SaveTROverDue_JSON_rsp, dynamic>(
                    storedProcedure: "usp_pIMTR_TROverDue_Save",
                    param);
                ...
                if (Resp > 0)
                {
                    IMTR_SaveTROverDue_JSON_rsp jsonResponse = JsonSerializer.Deserialize<IMTR_SaveTROverDue_JSON_rsp>(SaveResp);
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "Success";
        ...
        public async Task<ActionResult<IMTRResultResponse>> Release([FromBody] IMTR_ReleaseTROverDue_req release)
        {
            ...
            param.Add("@ExchRate", release.ExchRate); ;
            param.Add("@IntBalance", release.IntBalance);
            param.Add("@BLExch", release.BLExch);
            ...
            {
                await _db.SaveData(
                    storedProcedure: "usp_pIMTR_TROverDue_Release", param);
                ...
                {
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "T/R Overdue Release Complete";
                    return Ok(response);
                }
                else
                {
                    ...
                    {
                        response.Message = "T/R Overdue Release Error";
                    }
                    return BadRequest(response);
                }
            }
        ...
        [HttpPost("delete")]
        public async Task<ActionResult<IMTRResultResponse>> Delete([FromBody] IMTR_DeleteTROverDue_req delete)
        {
            IMTRResultResponse response = new();
            var USER_ID = User.Identity.Name;
            var claimsPrincipal = HttpContext.User;
            ...
            {
                await _db.SaveData(
                    storedProcedure: "usp_pIMTR_TROverDue_Delete", param);
                ...
                {
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "T/R Overdue Delete Complete";
                    return Ok(response);
                }
                else
                {
                    ...
                    {
                        response.Message = "T/R Overdue Delete Error";
                    }
                    return BadRequest(response);
                }
            }
