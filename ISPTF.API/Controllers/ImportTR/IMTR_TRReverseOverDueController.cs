...
namespace ISPTF.API.Controllers.ImportTR
{
    ...
    public class IMTR_TRReverseOverDueController : ControllerBase
    {
        private readonly ISqlDataAccess _db;
        private readonly ISPTFContext _context;
        public IMTR_TRReverseOverDueController(ISqlDataAccess db, ISPTFContext context)
        {
            _db = db;
            _context = context;
        }
        ...
        public async Task<ActionResult<Q_IMTR_ListPage_Response>> ListPage(string? ListType, string CustCode,string? CustName,string? TRNumber ,string? CenterID, string? Page, string? PageSize)
        {
            ...
            try
            {
                ...
                var results = await _db.LoadData<Q_IMTR_ListPage_rsp, dynamic>(
                            storedProcedure: "usp_q_IMTR_TRReverseOverDueListPage",
                            param);
        ...
        [HttpGet("Select")]
        public async Task<ActionResult<Q_IMTR_TRReverseOverDueSelect_Response>> Select(string? CustCode, string? RefNumber, string? TRSeqno, string? RecType, string? Event)
        {
            Q_IMTR_TRReverseOverDueSelect_Response response = new Q_IMTR_TRReverseOverDueSelect_Response();
            var USER_ID = User.Identity.Name;
            //var USER_ID = "API";
            // Validate
                ...
                string.IsNullOrEmpty(RecType) || string.IsNullOrEmpty(Event))
            {
                ...
                response.Data = new Q_IMTR_TRReverseOverDueSelect_JSON_rsp();
                return BadRequest(response);
            }
            ...
            try
            {
                ...
                param.Add("@TRReverseOverDueResp", dbType: DbType.String,
                           direction: System.Data.ParameterDirection.Output,
                           size: 5215585);


                var results = await _db.LoadData<Q_IMTR_TRReverseOverDueSelect_JSON_rsp, dynamic>(
                            storedProcedure: "usp_Q_IMTR_TRReverseOverDueListSelect",
                            param);

                var Resp = param.Get<dynamic>("@Resp");
                var TRReverseOverDueResp = param.Get<dynamic>("@TRReverseOverDueResp");

                if (Resp == 1)
                {
                    Q_IMTR_TRReverseOverDueSelect_JSON_rsp jsonResponse = JsonSerializer.Deserialize<Q_IMTR_TRReverseOverDueSelect_JSON_rsp>(TRReverseOverDueResp);
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "Success";
        ...
        [HttpPost("save")]
        public async Task<ActionResult<IMTR_SaveTRReverseOverDue_Response>> Save([FromBody] IMTR_SaveTRReverseOverDue_JSON_req save)
        {
            IMTR_SaveTRReverseOverDue_Response response = new();
            var USER_ID = User.Identity.Name;
            // Class validate
            if (save.ListType.ListType != "NEW" && save.ListType.ListType != "EDIT")
            {
                ...
                response.Data = new IMTR_SaveTRReverseOverDue_JSON_rsp();
                return BadRequest(response);
            }
            try
            {
                ...
                var results = await _db.LoadData<IMTR_SaveTRReverseOverDue_JSON_rsp, dynamic>(
                    storedProcedure: "usp_pIMTR_TRReverseOverDue_Save",
                    param);
                ...
                if (Resp > 0)
                {
                    IMTR_SaveTRReverseOverDue_JSON_rsp jsonResponse = JsonSerializer.Deserialize<IMTR_SaveTRReverseOverDue_JSON_rsp>(SaveResp);
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "Success";
        ...
        [HttpPost("release")]
        public async Task<ActionResult<IMTRResultResponse>> Release([FromBody] IMTR_ReleaseTRReverseOverDue_req release)
        {
            ...
            param.Add("@UserCode", USER_ID);
            param.Add("@OverdueDate", release.OverdueDate);
            ...
            param.Add("@IntBalance", release.IntBalance);
            ...
            {
                await _db.SaveData(
                    storedProcedure: "usp_pIMTR_TRReverseOverDue_Release", param);
                ...
                {
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "T/R Reverse OverDue Release Complete";
                    return Ok(response);
                }
                else
                {
                    ...
                    {
                        response.Message = "T/R Reverse OverDue Release Error";
                    }
                    return BadRequest(response);
                }
            }
        ...
        [HttpPost("delete")]
        public async Task<ActionResult<IMTRResultResponse>> Delete([FromBody] IMTR_DeleteTRReverseOverDue_req delete)
        {
            IMTRResultResponse response = new();
            var USER_ID = User.Identity.Name;
            // Class validate
            ...
            {
                await _db.SaveData(
                    storedProcedure: "usp_pIMTR_TRReverseOverDue_Delete", param);
                ...
                {
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "T/R Reverse OverDue Delete Complete";
                    return Ok(response);
                }
                else
                {
                    ...
                    {
                        response.Message = "T/R Reverse OverDue Delete Error";
                    }
                    return BadRequest(response);
                }
            }
