...
namespace ISPTF.API.Controllers.ImportTR
{
    ...
    public class IMTR_CollectRefundController : ControllerBase
    {
        private readonly ISqlDataAccess _db;
        private readonly ISPTFContext _context;
        public IMTR_CollectRefundController(ISqlDataAccess db, ISPTFContext context)
        {
            _db = db;
            _context = context;
        }
        ...
        public async Task<ActionResult<Q_IMTR_ListPage_Response>> ListPage(string? ListType, string CustCode,string? CustName,string? TRNumber ,string? CenterID, string? Page, string? PageSize)
        {
            ...
            try
            {
                ...
                var results = await _db.LoadData<Q_IMTR_ListPage_rsp, dynamic>(
                            storedProcedure: "usp_q_IMTR_CollectRefundListPage",
                            param);
        ...
        [HttpGet("Select")]
        public async Task<ActionResult<Q_IMTR_CollectRefundSelect_Response>> Select(string? CustCode, string? RefNumber, string? TRSeqno, string? RecType, string? Event)
        {
            Q_IMTR_CollectRefundSelect_Response response = new Q_IMTR_CollectRefundSelect_Response();
            var USER_ID = User.Identity.Name;
            //var USER_ID = "API";
            // Validate
                ...
                string.IsNullOrEmpty(RecType) || string.IsNullOrEmpty(Event))
            {
                ...
                response.Data = new Q_IMTR_CollectRefundSelect_JSON_rsp();
                return BadRequest(response);
            }
            ...
            try
            {
                ...
                param.Add("@CollectRefundResp", dbType: DbType.String,
                           direction: System.Data.ParameterDirection.Output,
                           size: 5215585);


                var results = await _db.LoadData<Q_IMTR_CollectRefundSelect_JSON_rsp, dynamic>(
                            storedProcedure: "usp_Q_IMTR_CollectRefundListSelect",
                            param);

                var Resp = param.Get<dynamic>("@Resp");
                var CollectRefundResp = param.Get<dynamic>("@CollectRefundResp");

                if (Resp == 1)
                {
                    Q_IMTR_CollectRefundSelect_JSON_rsp jsonResponse = JsonSerializer.Deserialize<Q_IMTR_CollectRefundSelect_JSON_rsp>(CollectRefundResp);
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "Success";
        ...
        [HttpPost("save")]
        public async Task<ActionResult<IMTR_SaveCollectRefund_Response>> Save([FromBody] IMTR_SaveCollectRefund_JSON_req save)
        {
            IMTR_SaveCollectRefund_Response response = new();
            var USER_ID = User.Identity.Name;
            // Class validate
            if (save.ListType.ListType != "NEW" && save.ListType.ListType != "EDIT")
            {
                ...
                response.Data = new IMTR_SaveCollectRefund_JSON_rsp();
                return BadRequest(response);
            }
            try
            {
                ...
                param.Add("@OpCollect", save.ListType.OpCollect);
                ...
                var results = await _db.LoadData<IMTR_SaveReverseTR_JSON_rsp, dynamic>(
                    storedProcedure: "usp_pIMTR_CollectRefund_Save",
                    param);
                ...
                if (Resp > 0)
                {
                    IMTR_SaveCollectRefund_JSON_rsp jsonResponse = JsonSerializer.Deserialize<IMTR_SaveCollectRefund_JSON_rsp>(SaveResp);
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "Success";
        ...
        [HttpPost("release")]
        public async Task<ActionResult<IMTRResultResponse>> Release([FromBody] IMTR_ReleaseCollectRefund_JSON_req release)
        {
            ...
            param.Add("@PostageAmt", release.pIMTR.PostageAmt);
            ...
            param.Add("@TaxRefund", release.pIMTR.TaxRefund);
            param.Add("@CommDesc", release.pIMTR.CommDesc);
            param.Add("@PayFlag", release.pIMTR.PayFlag);
            param.Add("@PayMethod", release.pIMTR.PayMethod);
            param.Add("@LastReceiptNo", release.pIMTR.LastReceiptNo);
            ...
            {
                await _db.SaveData(
                    storedProcedure: "usp_pIMTR_CollectRefund_Release", param);
        ...
        [HttpPost("delete")]
        public async Task<ActionResult<IMTRResultResponse>> Delete([FromBody] IMTR_DeleteCollectRefund_JSON_req delete)
        {
            IMTRResultResponse response = new();
            var USER_ID = User.Identity.Name;
            // Class validate
            ...
            param.Add("@LastReceiptNo", delete.pIMTR.LastReceiptNo);
            ...
            {
                await _db.SaveData(
                    storedProcedure: "usp_pIMTR_CollectRefund_Delete", param);
