...
namespace ISPTF.API.Controllers.ImportTR
{
    ...
    public class IMTR_ChangeIntRateController : ControllerBase
    {
        ...
        [HttpPost("save")]
        public async Task<ActionResult<IMTR_SaveChangeIntRate_Response>> Save([FromBody] IMTR_SaveChangeIntRate_JSON_req save)
        {
            ...
            if (save.ListType.ListType != "NEW" && save.ListType.ListType != "EDIT")
            {
                ...
                response.Data = new IMTR_SaveChangeIntRate_JSON_rsp();
                return BadRequest(response);
            }
            try
            {
                ...
                var results = await _db.LoadData<IMTR_SaveChangeIntRate_Response, dynamic>(
                    storedProcedure: "usp_pIMTR_ChangeIntRate_Save",
                    param);
                ...
                if (Resp > 0)
                {
                    IMTR_SaveChangeIntRate_JSON_rsp jsonResponse = JsonSerializer.Deserialize<IMTR_SaveChangeIntRate_JSON_rsp>(SaveResp);
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "Success";
        ...
        [HttpPost("release")]
        public async Task<ActionResult<IMTRResultResponse>> Release([FromBody] IMTR_ReleaseChangeIntRate_req release)
        {
            ...
            param.Add("@SettleDate", release.SettleDate); ;
            ...
            {
                await _db.SaveData(
                    storedProcedure: "usp_pIMTR_ChangeIntRate_Release", param);
                ...
                {
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "Change Interest Rate Release Complete";
                    return Ok(response);
                }
                else
                {
                    ...
                    {
                        response.Message = "Change Interest Rate Release Error";
                    }
                    return BadRequest(response);
                }
            }
        ...
        [HttpPost("delete")]
        public async Task<ActionResult<IMTRResultResponse>> Delete([FromBody] IMTR_DeleteChangeIntRate_req delete)
        {
            IMTRResultResponse response = new();
            var USER_ID = User.Identity.Name;
            // Class validate
            ...
            {
                await _db.SaveData(
                    storedProcedure: "usp_pIMTR_ChangeIntRate_Delete", param);
                ...
                {
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "Change Interest Rate Delete Complete";
                    return Ok(response);
                }
                else
                {
                    ...
                    {
                        response.Message = "Change Interest Rate Delete Error";
                    }
                    return BadRequest(response);
                }
            }
