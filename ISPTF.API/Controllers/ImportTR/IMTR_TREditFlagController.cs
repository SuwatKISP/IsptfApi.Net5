...
namespace ISPTF.API.Controllers.ImportTR
{
    ...
    public class IMTR_TREditFlagController : ControllerBase
    {
        private readonly ISqlDataAccess _db;
        private readonly ISPTFContext _context;
        public IMTR_TREditFlagController(ISqlDataAccess db, ISPTFContext context)
        {
            _db = db;
            _context = context;
        }
        ...
        [HttpGet("listpage")]
        public async Task<ActionResult<Q_IMTR_EditFlag_ListPage_Response>> ListPage(string? ListType, string CustCode,string? CustName,string? TRNumber ,string? CenterID, string? Page, string? PageSize)
        {
            Q_IMTR_EditFlag_ListPage_Response response = new Q_IMTR_EditFlag_ListPage_Response();
            ...
            if (string.IsNullOrEmpty(ListType) || string.IsNullOrEmpty(CenterID) || string.IsNullOrEmpty(Page) || string.IsNullOrEmpty(PageSize))
            {
                ...
                response.Data = new List<Q_IMTR_EditFlag_ListPage_rsp>();
            ...
            try
            {
                ...
                var results = await _db.LoadData<Q_IMTR_EditFlag_ListPage_rsp, dynamic>(
                            storedProcedure: "usp_q_IMTR_TREditFlagListPage",
                            param);

                response.Code = Constants.RESPONSE_OK;
                response.Message = "Success";
                response.Data = (List<Q_IMTR_EditFlag_ListPage_rsp>)results;
        ...
        [HttpGet("Select")]
<<<<<<< HEAD
        public async Task<ActionResult<Q_IMTR_TREditFlagSelect_Response>> Select(string? CustCode, string? RefNumber, string? TRSeqno)
=======
        public async Task<ActionResult<Q_IMTR_TREditFlagSelect_Response>> Select(string? ListType, string? CustCode, string? RefNumber, string? TRSeqno)
>>>>>>> c45d0d5b37e973cf630f6278a185e50b4ca8217d
        {
            Q_IMTR_TREditFlagSelect_Response response = new Q_IMTR_TREditFlagSelect_Response();
            var USER_ID = User.Identity.Name;
            //var USER_ID = "API";
            // Validate
            ...
            {
                response.Code = Constants.RESPONSE_FIELD_REQUIRED;
                response.Message = "CustCode, RefNumber, TRSeqno are required";
                response.Data = new Q_IMTR_TREditFlagSelect_JSON_rsp();
                return BadRequest(response);
            }
<<<<<<< HEAD
...
>>>>>>> c45d0d5b37e973cf630f6278a185e50b4ca8217d
                ...
                param.Add("@TREditFlagResp", dbType: DbType.String,
                           direction: System.Data.ParameterDirection.Output,
                           size: 5215585);


                var results = await _db.LoadData<Q_IMTR_TREditFlagSelect_JSON_rsp, dynamic>(
                            storedProcedure: "usp_Q_IMTR_TREditFlagListSelect",
                            param);

                var Resp = param.Get<dynamic>("@Resp");
                var TREditFlagResp = param.Get<dynamic>("@TREditFlagResp");

                if (Resp == 1)
                {
                    Q_IMTR_TREditFlagSelect_JSON_rsp jsonResponse = JsonSerializer.Deserialize<Q_IMTR_TREditFlagSelect_JSON_rsp>(TREditFlagResp);
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "Success";
...
=======
        [HttpPost("save")]
        public async Task<ActionResult<IMTR_SaveTREditFlag_Response>> Save([FromBody] IMTR_SaveTREditFlag_JSON_req save)
        {
            IMTR_SaveTREditFlag_Response response = new();
            var USER_ID = User.Identity.Name;
            // Class validate
            if (save.ListType.ListType != "NEW" && save.ListType.ListType != "EDIT")
            {
                ...
                response.Data = new IMTR_SaveTREditFlag_JSON_rsp();
                return BadRequest(response);
            }
            try
            {
                ...
                var results = await _db.LoadData<IMTR_SaveTREditFlag_JSON_rsp, dynamic>(
                    storedProcedure: "usp_pIMTR_TREditFlag_Save",
                    param);
                ...
                if (Resp > 0)
                {
                    IMTR_SaveTREditFlag_JSON_rsp jsonResponse = JsonSerializer.Deserialize<IMTR_SaveTREditFlag_JSON_rsp>(SaveResp);
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "Success";
        ...
        [HttpPost("release")]
        public async Task<ActionResult<IMTRResultResponse>> Release([FromBody] IMTR_ReleaseTREditFlag_JSON_req release)
        {
            ...
            param.Add("@CenterID", release.pIMTR.CenterID);
            param.Add("@UserCode", release.pIMTR.UserCode);
            param.Add("@IntStartDate", release.pIMTR.IntStartDate);
            param.Add("@IntPayType", release.pIMTR.IntPayType);
            param.Add("@IntRateCode", release.pIMTR.IntRateCode);
            param.Add("@IntRate", release.pIMTR.IntRate);
            param.Add("@IntSpread", release.pIMTR.IntSpread);
            param.Add("@AutoOverDue", release.pIMTR.AutoOverDue);
            param.Add("@NegoTelex", release.pIMTR.NegoTelex);
            param.Add("@IntFlag", release.pIMTR.IntFlag);
            param.Add("@IntBaseDay", release.pIMTR.IntBaseDay);
            param.Add("@DueDate", release.pIMTR.DueDate);
            ...
            {
                await _db.SaveData(
                    storedProcedure: "usp_pIMTR_TREditFlag_Release", param);
        ...
        [HttpPost("delete")]
        public async Task<ActionResult<IMTRResultResponse>> Delete([FromBody] IMTR_DeleteTREditFlag_JSON_req delete)
        {
            IMTRResultResponse response = new();
            var USER_ID = User.Identity.Name;
            // Class validate
            ...
            {
                await _db.SaveData(
                    storedProcedure: "usp_pIMTR_TREditFlag_Delete", param);
