...
=======
        [HttpPost("save")]
        public async Task<ActionResult<IMTR_SaveReverseTR_Response>> Save([FromBody] IMTR_SaveReverseTR_JSON_req save)
        {
            IMTR_SaveReverseTR_Response response = new();
            var USER_ID = User.Identity.Name;
            // Class validate
            if (save.ListType.ListType != "NEW" && save.ListType.ListType != "EDIT")
            {
                ...
                response.Data = new IMTR_SaveReverseTR_JSON_rsp();
                return BadRequest(response);
            }
            try
            {
                ...
                param.Add("@LbTR", save.ListType.LbTR);
                param.Add("@TextCRAmt", save.ListType.TextCRAmt);
                ...
                var results = await _db.LoadData<IMTR_SaveReverseTR_JSON_rsp, dynamic>(
                    storedProcedure: "usp_pIMTR_ReverseTR_Save",
                    param);
                ...
                if (Resp > 0)
                {
                    IMTR_SaveReverseTR_JSON_rsp jsonResponse = JsonSerializer.Deserialize<IMTR_SaveReverseTR_JSON_rsp>(SaveResp);
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "Success";
        ...
        [HttpPost("release")]
        public async Task<ActionResult<IMTRResultResponse>> Release([FromBody] IMTR_ReleaseReverseTR_JSON_req release)
        {
            ...
            param.Add("@LbTR", release.ListType.LbTR);
            ...
            param.Add("@BLInterest", release.pIMTR.BLInterest);
            param.Add("@TRCCyFlag", release.pIMTR.TRCCyFlag);
            param.Add("@BLIntAmt", release.pIMTR.BLIntAmt);
            ...
            {
                await _db.SaveData(
                    storedProcedure: "usp_pIMTR_ReverseTR_Release", param);
        ...
        [HttpPost("delete")]
        public async Task<ActionResult<IMTRResultResponse>> Delete([FromBody] IMTR_DeleteReverseTR_JSON_req delete)
        {
            IMTRResultResponse response = new();
            var USER_ID = User.Identity.Name;
            // Class validate
            ...
            {
                await _db.SaveData(
                    storedProcedure: "usp_pIMTR_ReverseTR_Delete", param);
