...
namespace ISPTF.API.Controllers.ImportTR
{
    ...
    public class IMTR_ConvertTRController : ControllerBase
    {
        private readonly ISqlDataAccess _db;
        private readonly ISPTFContext _context;
        public IMTR_ConvertTRController(ISqlDataAccess db, ISPTFContext context)
        {
            _db = db;
            _context = context;
        }
        ...
        public async Task<ActionResult<Q_IMTR_ListPage_Response>> ListPage(string? ListType, string CustCode,string? CustName,string? TRNumber ,string? CenterID, string? Page, string? PageSize)
        {
            ...
            try
            {
                ...
                var results = await _db.LoadData<Q_IMTR_ListPage_rsp, dynamic>(
                            storedProcedure: "usp_q_IMTR_ConvertTRListPage",
                            param);
        ...
        public async Task<ActionResult<Q_IMTR_ConvertTRSelect_Response>> Select(string? CustCode, string? RefNumber, string? TRSeqno, string? RecType, string? Event)
        {
            ...
            try
            {
                ...
                param.Add("@ConvertTRResp", dbType: DbType.String,
                           direction: System.Data.ParameterDirection.Output,
                           size: 5215585);


                var results = await _db.LoadData<Q_IMTR_ConvertTRSelect_JSON_rsp, dynamic>(
                            storedProcedure: "usp_Q_IMTR_ConvertTRListSelect",
                            param);

                var Resp = param.Get<dynamic>("@Resp");
                var ConvertTRResp = param.Get<dynamic>("@ConvertTRResp");

                if (Resp == 1)
                {
                    Q_IMTR_ConvertTRSelect_JSON_rsp jsonResponse = JsonSerializer.Deserialize<Q_IMTR_ConvertTRSelect_JSON_rsp>(ConvertTRResp);
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "Success";
        ...
        [HttpPost("save")]
        public async Task<ActionResult<IMTR_SaveConvert_Response>> Save([FromBody] IMTR_SaveConvert_JSON_req save)
        {
            ...
            if (save.ListType.ListType != "NEW" && save.ListType.ListType != "EDIT")
            {
                ...
                response.Data = new IMTR_SaveConvert_JSON_rsp();
                return BadRequest(response);
            }
            try
            {
                ...
                var results = await _db.LoadData<IMTR_SaveConvert_Response, dynamic>(
                    storedProcedure: "usp_pIMTR_ConvertTR_Save",
                    param);
                ...
                if (Resp > 0)
                {
                    IMTR_SaveConvert_JSON_rsp jsonResponse = JsonSerializer.Deserialize<IMTR_SaveConvert_JSON_rsp>(SaveResp);
                    response.Code = Constants.RESPONSE_OK;
                    response.Message = "Success";
