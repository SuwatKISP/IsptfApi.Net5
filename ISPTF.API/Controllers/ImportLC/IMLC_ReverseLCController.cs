...
namespace ISPTF.API.Controllers.ImportLC
{
    ...
    public class IMLC_ReverseLCController : ControllerBase
    {
        private readonly ISqlDataAccess _db;
        private readonly ISPTFContext _context;
        public IMLC_ReverseLCController(ISqlDataAccess db, ISPTFContext context)
        {
            _db = db;
            _context = context;
        }
        ...
        [HttpGet("listpage")]
        public async Task<ActionResult<Q_IMLC_ReverseLC_ListPage_Response>> ListPage(string? ListType, string? LCNumber, string CustCode, string? CustName, string? CenterID, string? Page, string? PageSize)
        {
            Q_IMLC_ReverseLC_ListPage_Response response = new Q_IMLC_ReverseLC_ListPage_Response();
            ...
            if (string.IsNullOrEmpty(ListType) || string.IsNullOrEmpty(CenterID) || string.IsNullOrEmpty(Page) || string.IsNullOrEmpty(PageSize))
            {
                ...
                response.Data = new List<Q_IMLC_ReverseLC_ListPage_rsp>();
            ...
            try
            {
                ...
                var results = await _db.LoadData<Q_IMLC_ReverseLC_ListPage_rsp, dynamic>(
                            storedProcedure: "usp_q_IMLC_ReverseLCListPage",
                            param);

                response.Code = Constants.RESPONSE_OK;
                response.Message = "Success";
                response.Data = (List<Q_IMLC_ReverseLC_ListPage_rsp>)results;
