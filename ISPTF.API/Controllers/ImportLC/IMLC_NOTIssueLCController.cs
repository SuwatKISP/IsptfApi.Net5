...
namespace ISPTF.API.Controllers.ImportLC
{
    ...
    public class IMLC_NOTIssueLCController : ControllerBase
    {
        private readonly ISqlDataAccess _db;
        private readonly ISPTFContext _context;
        public IMLC_NOTIssueLCController(ISqlDataAccess db, ISPTFContext context)
        {
            _db = db;
            _context = context;
        }
        ...
        [HttpPost("save")]
        public async Task<ActionResult<IMLC_SaveNOTIssue_Response>> Save([FromBody] IMLC_SaveNOTIssue_JSON_req save)
        {
            ...
            try
            {
                ...
                var results = await _db.LoadData<IMLC_SaveNOTIssue_JSON_rsp, dynamic>(
                    storedProcedure: "usp_pIMLC_NOTIssue_Save",
                    param);
                ...
                if (Resp > 0)
                {
                    IMLC_SaveNOTIssue_JSON_rsp jsonResponse = JsonSerializer.Deserialize<IMLC_SaveNOTIssue_JSON_rsp>(SaveResp);
        ...
        [HttpPost("release")]
        public async Task<ActionResult<IMLCResultResponse>> Release([FromBody] IMLC_ReleaseNOTIssue_JSON_req release)
        {
            ...
            {
                await _db.SaveData(
                  storedProcedure: "usp_pIMLC_NOTIssue_Release", param);
                var resp = param.Get<int>("@Resp");
