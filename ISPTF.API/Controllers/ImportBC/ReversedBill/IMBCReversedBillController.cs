...
namespace ISPTF.API.Controllers.ImportBC
{
    ...
    public class IMBCCollectReversedBillController : ControllerBase
    {
        private readonly ISqlDataAccess _db;
        public IMBCCollectReversedBillController(ISqlDataAccess db)
        {
            _db = db;
        }

        [HttpGet("listpage")]
        public async Task<IEnumerable<PIMBCListRsp>> GetAll(string? ListType ,string? CenterID, string? BCNumber, string? CustCode, string? CustName,string? UserCode, string? Page, string? PageSize, string? RegDocno)
        {
        ...
        public async Task<ActionResult<PIMBCPPaymentRsp>> GetAllSelect(string BCNumber, string BCSeqno, string RecType, string EVENT, string RecStatus)
        {
            ...
            try
            {
                var results = await _db.LoadData<PIMBCPPaymentRsp, dynamic>(
                           storedProcedure: "usp_pIMBC_Reverse_Select",
                           param);
                ...
                else
                {
                    ...
                    response.Message = "IMPORT B/C NO for Reverse does not exit";
        ...
        [HttpGet("select/payment")]
        public async Task<ActionResult<pPayment>> GetPayment(string LastReceiptNo)
        {
            DynamicParameters param = new();

            param.Add("@LastReceiptNo", LastReceiptNo);

            param.Add("@ReceiptRsp", dbType: DbType.Int32,
                       direction: ParameterDirection.Output,
                       size: 12800);

            param.Add("@ReceiptRspInfo", dbType: DbType.String,
                       direction: ParameterDirection.Output,
                       size: 5215585);
            ...
            {
                var results = await _db.LoadData<PIMBCPPaymentRsp, dynamic>(
                           storedProcedure: "usp_pIMBC_Reverse_Select_Payment",
                           param);

                var ReceiptRsp = param.Get<dynamic>("@ReceiptRsp");
                var ReceiptRspInfo = param.Get<dynamic>("@ReceiptRspInfo");

                if (ReceiptRsp > 0)
                {
                    return Ok(ReceiptRspInfo);
                }
                else
                {
                    ...
                    ReturnResponse response = new();
                    response.StatusCode = "400";
                    response.Message = "IMPORT Payment Receipt NO does not exit";
                    return BadRequest(response);
                }
        ...
        public async Task<ActionResult<List<PIMBCPPaymentRsp>>> Insert([FromBody] PIMBCPPaymentRsp pimbcrsp)
        {
            ...
            {
                var results = await _db.LoadData<PIMBCPPaymentRsp, dynamic>(
                    storedProcedure: "usp_pIMBC_Reverse_Insert",
                    param);
        ...
        public async Task<ActionResult<List<PIMBCPPaymentRsp>>> Update([FromBody] PIMBCPPaymentRsp pimbcrsp)
        {
            ...
            {
                var results = await _db.LoadData<PIMBCPPaymentRsp, dynamic>(
                    storedProcedure: "usp_pIMBC_Reverse_Update",
                    param);
        ...
        public async Task<ActionResult<List<PIMBCPPaymentRsp>>> GetAllRelease([FromBody] PIMBCReleaseReq pimbcrsp)
        {
            ...
            {
                var results = await _db.LoadData<PIMBCPPaymentRsp, dynamic>(
                    storedProcedure: "usp_pIMBC_Reverse_Release",
                    param);
                var PIMBCRsp = param.Get<dynamic>("@PIMBCRsp");
                var PIMBCPPaymentRsp = param.Get<dynamic>("@PIMBCPPaymentRsp");
                var resp = param.Get<int>("@Resp");
        ...
        public async Task<ActionResult<List<PIMBCMasterDeleteReq>>> GetAllDelete(string BCNumber, string BCSeqno, DateTime? EventDate)
        {
            ...
            try
            {
                var results = await _db.LoadData<PDocRegister, dynamic>(
                    storedProcedure: "usp_pIMBC_Reverse_Delete",
                    param);
                var resp = param.Get<int>("@Resp");
                if (resp == 1)
                {
